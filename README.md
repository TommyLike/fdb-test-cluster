...
```
when cluster is deployed, you can login the grafana within `admin` user and the specified password.

There would be a `FoundationDB Cluster` Dashboard in default and it's graphs are configured as below:
```$xslt
     +--------------------------------------------------+
     |              process cpu usage                   |
     +--------------------------------------------------+
